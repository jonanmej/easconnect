...
import { ProgramacionDoc } from "../src/lib/pdf/ProgramacionDoc";
...
const mkSem = (n:number, dias:any[]) => ({semana_numero:n, dias});
const D = (fecha:string,in_month:boolean,feriado:boolean,items:any[]=[]) => ({fecha,in_month,feriado,items});
const ev = (planta:string, servicio:string, di:number, du:number, estado='programado') => ({id:'x',folio:'',servicio,estado,planta_nombre:planta,dia_idx:di,duracion:du});
...
async function make(name:string, data:any){
  const stream:any = await pdf(React.createElement(ProgramacionDoc,{data})).toBuffer();
  const chunks:any[]=[]; for await (const c of stream) chunks.push(c);
