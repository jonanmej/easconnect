-- 1) Columnas de moneda/impuesto por OC y por línea de recepción
ALTER TABLE public.ordenes_compra
  ADD COLUMN IF NOT EXISTS moneda text NOT NULL DEFAULT 'USD',
  ADD COLUMN IF NOT EXISTS impuesto_pct numeric NOT NULL DEFAULT 0;

ALTER TABLE public.orden_compra_recepcion_items
  ADD COLUMN IF NOT EXISTS moneda text NOT NULL DEFAULT 'USD',
  ADD COLUMN IF NOT EXISTS impuesto_pct numeric NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS precio_esperado numeric,
  ADD COLUMN IF NOT EXISTS variacion_motivo text;

-- Validaciones defensivas: sin negativos ni ceros
ALTER TABLE public.orden_compra_recepcion_items
  DROP CONSTRAINT IF EXISTS chk_recep_cantidad_pos;
ALTER TABLE public.orden_compra_recepcion_items
  ADD CONSTRAINT chk_recep_cantidad_pos CHECK (cantidad > 0);

ALTER TABLE public.orden_compra_recepcion_items
  DROP CONSTRAINT IF EXISTS chk_recep_costo_nonneg;
ALTER TABLE public.orden_compra_recepcion_items
  ADD CONSTRAINT chk_recep_costo_nonneg CHECK (costo_unitario >= 0);

-- 2) Auditoría de variaciones (precio/moneda/impuesto)
CREATE TABLE IF NOT EXISTS public.orden_compra_variaciones (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  recepcion_item_id uuid NOT NULL REFERENCES public.orden_compra_recepcion_items(id) ON DELETE CASCADE,
  orden_id uuid NOT NULL REFERENCES public.ordenes_compra(id) ON DELETE CASCADE,
  tipo text NOT NULL CHECK (tipo IN ('precio','moneda','impuesto','cantidad')),
  valor_esperado text,
  valor_recibido text,
  motivo text,
  registrado_por uuid,
...
GRANT SELECT, INSERT ON public.orden_compra_variaciones TO authenticated;
GRANT ALL ON public.orden_compra_variaciones TO service_role;

ALTER TABLE public.orden_compra_variaciones ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Staff ve variaciones" ON public.orden_compra_variaciones
  FOR SELECT TO authenticated
  USING (public.has_role(auth.uid(),'admin') OR public.has_role(auth.uid(),'supervisor') OR public.has_role(auth.uid(),'tecnico'));

CREATE POLICY "Staff registra variaciones" ON public.orden_compra_variaciones
...
CREATE INDEX IF NOT EXISTS idx_oc_variaciones_orden ON public.orden_compra_variaciones(orden_id);
CREATE INDEX IF NOT EXISTS idx_oc_variaciones_recep ON public.orden_compra_variaciones(recepcion_item_id);

-- 3) Reforzar registrar_recepcion_oc con validaciones y captura de variaciones
...
BEGIN
  ...
  FOR v_line IN SELECT * FROM jsonb_array_elements(_lineas) LOOP
    ...
    -- Precio esperado por defecto: precio_unitario de la orden
    IF v_precio_esperado IS NULL THEN
      v_precio_esperado := v_orden_item.precio_unitario;
    END IF;

    -- Si hay variación de precio, moneda o impuesto: exigir motivo
    ...
    -- Neutralizar doble suma del trigger apply_movimiento
    ...
    -- Registrar auditoría de variaciones
...
-- 4) Edición reconciliada de una línea de recepción
CREATE OR REPLACE FUNCTION public.editar_recepcion_item_oc(
  _recepcion_item_id uuid,
  _nueva_cantidad numeric,
  _nuevo_costo numeric,
  _motivo text
)
RETURNS void
LANGUAGE plpgsql
...
BEGIN
  ...
  -- Validar sobrerrecepción con el delta
  ...
  -- Ajustar stock y costo promedio del inventario reconciliando el delta
  SELECT stock_actual, COALESCE(costo_promedio,0) INTO v_prev_stock, v_prev_avg
  FROM public.inventario_items WHERE id = v_ri.item_id FOR UPDATE;
  IF v_prev_stock IS NULL THEN RAISE EXCEPTION 'Ítem de inventario no existe'; END IF;

  -- Revertir el aporte de esta línea (usando los valores originales)
  ...
  END IF;
  -- Aplicar la nueva cantidad/costo con promedio ponderado
  ...
  -- Movimiento auditable del ajuste
  INSERT INTO public.inventario_movimientos(item_id, tipo, cantidad, motivo, ref_trabajo_id)
  ...
  -- Actualizar totales de la línea de OC
  UPDATE public.orden_compra_items
  ...
  -- Reconciliar estado de la orden
  DECLARE
